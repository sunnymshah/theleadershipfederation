/**
 * Badge/Name Tag PDF Generation
 *
 * Generates printable A6 landscape badges (4 per A4 sheet, 2x2 grid).
 * Accepts a BadgeDesign configuration for full visual customization.
 * Works server-side in Node.js (no browser APIs required).
 *
 * Badge layout (A6 = 148mm x 105mm landscape, 105mm x 148mm portrait):
 *   - TLF logo placeholder (accent rectangle, top center)
 *   - Attendee name (LARGE, centered, bold, configurable size)
 *   - Company + Designation (medium, below name, toggleable)
 *   - VIP/Speaker/Sponsor color-coded ribbon (toggleable)
 *   - QR code placeholder (bottom right, toggleable)
 *   - Badge number (bottom left)
 *   - Configurable background, accent, border
 */

import { jsPDF } from "jspdf"

/* ── Types ──────────────────────────────────────────────────────────── */

export interface BadgeDesign {
  layout: "landscape" | "portrait"
  bgColor: string
  accentColor: string
  textColor: string
  nameSize: number
  showCompany: boolean
  showDesignation: boolean
  showVip: boolean
  showQr: boolean
  borderStyle: "none" | "thin" | "gold" | "double"
  eventTitle: string
}

export interface BadgeData {
  attendeeName: string
  company?: string | null
  designation?: string | null
  qrToken: string
  badgeNumber: number
  vipLevel?: string | null
}

/* ── Defaults ───────────────────────────────────────────────────────── */

export const DEFAULT_DESIGN: BadgeDesign = {
  layout: "landscape",
  bgColor: "#ffffff",
  accentColor: "#e7ab1c",
  textColor: "#1e1e1e",
  nameSize: 32,
  showCompany: true,
  showDesignation: true,
  showVip: true,
  showQr: true,
  borderStyle: "gold",
  eventTitle: "The Leadership Federation",
}

/* ── Color map for VIP levels ─────────────────────────────────────── */

const VIP_COLORS: Record<string, { bg: [number, number, number]; text: [number, number, number]; label: string }> = {
  vip:     { bg: [231, 171, 28],  text: [255, 255, 255], label: "VIP" },
  vvip:    { bg: [180, 50, 50],   text: [255, 255, 255], label: "VVIP" },
  speaker: { bg: [50, 100, 180],  text: [255, 255, 255], label: "SPEAKER" },
  sponsor: { bg: [40, 140, 80],   text: [255, 255, 255], label: "SPONSOR" },
  media:   { bg: [120, 60, 160],  text: [255, 255, 255], label: "MEDIA" },
}

/* ── Helpers ──────────────────────────────────────────────────────── */

function hexToRgb(hex: string): [number, number, number] {
  const h = hex.replace("#", "")
  return [
    parseInt(h.substring(0, 2), 16),
    parseInt(h.substring(2, 4), 16),
    parseInt(h.substring(4, 6), 16),
  ]
}

/** Determine if a color is "dark" using luminance */
function isDark(hex: string): boolean {
  const [r, g, b] = hexToRgb(hex)
  const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255
  return luminance < 0.5
}

/* ── Badge dimensions ────────────────────────────────────────────── */

function getBadgeDimensions(layout: "landscape" | "portrait") {
  return layout === "landscape"
    ? { w: 148, h: 105 }
    : { w: 105, h: 148 }
}

/* ── Draw a single badge ──────────────────────────────────────────── */

function drawBadge(
  doc: jsPDF,
  badge: BadgeData,
  design: BadgeDesign,
  offX: number,
  offY: number,
) {
  const { w: BADGE_W, h: BADGE_H } = getBadgeDimensions(design.layout)
  const accent = hexToRgb(design.accentColor)
  const bgRgb = hexToRgb(design.bgColor)
  const textRgb = hexToRgb(design.textColor)
  const darkBg = isDark(design.bgColor)
  const grayText: [number, number, number] = darkBg ? [180, 180, 180] : [120, 120, 120]
  const subtleText: [number, number, number] = darkBg ? [140, 140, 140] : [160, 160, 160]

  // ── Background fill ─────────────────────────────────────────────
  doc.setFillColor(...bgRgb)
  doc.rect(offX, offY, BADGE_W, BADGE_H, "F")

  // ── Border ──────────────────────────────────────────────────────
  if (design.borderStyle === "thin") {
    doc.setDrawColor(...grayText)
    doc.setLineWidth(0.3)
    doc.rect(offX, offY, BADGE_W, BADGE_H)
  } else if (design.borderStyle === "gold") {
    doc.setDrawColor(...accent)
    doc.setLineWidth(1)
    doc.rect(offX + 1.5, offY + 1.5, BADGE_W - 3, BADGE_H - 3)
  } else if (design.borderStyle === "double") {
    doc.setDrawColor(...accent)
    doc.setLineWidth(0.8)
    doc.rect(offX + 1, offY + 1, BADGE_W - 2, BADGE_H - 2)
    doc.setLineWidth(0.4)
    doc.rect(offX + 3, offY + 3, BADGE_W - 6, BADGE_H - 6)
  } else {
    // none — just cutting guide
    doc.setDrawColor(200, 200, 200)
    doc.setLineWidth(0.15)
    doc.rect(offX, offY, BADGE_W, BADGE_H)
  }

  // ── TLF Logo placeholder (accent rectangle at top center) ──────
  const logoW = Math.min(50, BADGE_W - 20)
  const logoH = 10
  const logoX = offX + (BADGE_W - logoW) / 2
  const logoY = offY + 10
  doc.setFillColor(...accent)
  doc.roundedRect(logoX, logoY, logoW, logoH, 2, 2, "F")

  doc.setFont("helvetica", "bold")
  doc.setFontSize(7)
  doc.setTextColor(255, 255, 255)
  doc.text("THE LEADERSHIP FEDERATION", offX + BADGE_W / 2, logoY + 6.5, { align: "center" })

  // ── Event title (small, below logo) ────────────────────────────
  doc.setFont("helvetica", "normal")
  doc.setFontSize(8)
  doc.setTextColor(...grayText)
  const eventTitleLines = doc.splitTextToSize(design.eventTitle, BADGE_W - 20)
  const eventTitleY = logoY + logoH + 6
  doc.text(eventTitleLines.slice(0, 2), offX + BADGE_W / 2, eventTitleY, { align: "center" })

  // ── Accent divider line ────────────────────────────────────────
  const dividerY = eventTitleY + (eventTitleLines.length > 1 ? 8 : 4)
  doc.setDrawColor(...accent)
  doc.setLineWidth(0.6)
  doc.line(offX + 20, dividerY, offX + BADGE_W - 20, dividerY)

  // ── Attendee name (LARGE, centered, bold) ──────────────────────
  doc.setFont("helvetica", "bold")
  doc.setTextColor(...textRgb)

  const maxNameWidth = BADGE_W - 16
  let nameFontSize = design.nameSize * 0.65 // Convert pt → roughly mm-appropriate size
  if (nameFontSize < 12) nameFontSize = 12
  if (nameFontSize > 32) nameFontSize = 32
  doc.setFontSize(nameFontSize)

  // Shrink to fit
  while (doc.getTextWidth(badge.attendeeName) > maxNameWidth && nameFontSize > 10) {
    nameFontSize -= 1
    doc.setFontSize(nameFontSize)
  }

  const nameY = dividerY + 14
  doc.text(badge.attendeeName, offX + BADGE_W / 2, nameY, { align: "center" })

  // ── Company + Designation ──────────────────────────────────────
  let infoY = nameY + 8
  doc.setFont("helvetica", "normal")
  doc.setFontSize(9)
  doc.setTextColor(darkBg ? 200 : 80, darkBg ? 200 : 80, darkBg ? 200 : 80)

  if (design.showDesignation && badge.designation) {
    const desgLines = doc.splitTextToSize(badge.designation, BADGE_W - 20)
    doc.text(desgLines.slice(0, 1), offX + BADGE_W / 2, infoY, { align: "center" })
    infoY += 5
  }

  if (design.showCompany && badge.company) {
    doc.setFont("helvetica", "bold")
    doc.setFontSize(9)
    const compLines = doc.splitTextToSize(badge.company, BADGE_W - 20)
    doc.text(compLines.slice(0, 1), offX + BADGE_W / 2, infoY, { align: "center" })
    infoY += 5
  }

  // ── VIP ribbon (top-right corner) ──────────────────────────────
  if (design.showVip) {
    const vipLevel = badge.vipLevel?.toLowerCase() ?? ""
    const vipConfig = VIP_COLORS[vipLevel]

    if (vipConfig) {
      const ribbonW = 28
      const ribbonH = 8
      const ribbonX = offX + BADGE_W - ribbonW - 5
      const ribbonY = offY + 3

      doc.setFillColor(...vipConfig.bg)
      doc.roundedRect(ribbonX, ribbonY, ribbonW, ribbonH, 1.5, 1.5, "F")

      doc.setFont("helvetica", "bold")
      doc.setFontSize(7)
      doc.setTextColor(...vipConfig.text)
      doc.text(vipConfig.label, ribbonX + ribbonW / 2, ribbonY + 5.5, { align: "center" })
    }
  }

  // ── Bottom section ─────────────────────────────────────────────

  // Badge number (bottom left)
  doc.setFont("helvetica", "normal")
  doc.setFontSize(7)
  doc.setTextColor(...subtleText)
  doc.text(`#${badge.badgeNumber.toString().padStart(4, "0")}`, offX + 6, offY + BADGE_H - 6)

  // QR code placeholder (bottom right)
  if (design.showQr) {
    const qrBoxSize = 14
    const qrBoxX = offX + BADGE_W - 6 - qrBoxSize
    const qrBoxY = offY + BADGE_H - 8 - qrBoxSize
    doc.setDrawColor(...subtleText)
    doc.setLineWidth(0.3)
    doc.rect(qrBoxX, qrBoxY, qrBoxSize, qrBoxSize)
    doc.setFontSize(5)
    doc.setTextColor(...subtleText)
    doc.text("QR", qrBoxX + qrBoxSize / 2, qrBoxY + qrBoxSize / 2 + 1, { align: "center" })

    // Token text below QR box
    doc.setFontSize(5)
    const tokenShort = badge.qrToken.slice(0, 12)
    doc.text(tokenShort, offX + BADGE_W - 6, offY + BADGE_H - 6, { align: "right" })
  }

  // Bottom accent line
  doc.setDrawColor(...accent)
  doc.setLineWidth(1)
  doc.line(offX + 3, offY + BADGE_H - 3, offX + BADGE_W - 3, offY + BADGE_H - 3)
}

/* ── Generate multi-page badge PDF (4 per A4, 2x2 grid) ──────────── */

/**
 * Creates a multi-page PDF with 4 badges per page (2x2 grid on A4).
 * Returns the jsPDF instance for further manipulation or output.
 */
export function generateBadgePDF(badges: BadgeData[], design?: BadgeDesign): jsPDF {
  const config = design ?? DEFAULT_DESIGN
  const { w: BADGE_W, h: BADGE_H } = getBadgeDimensions(config.layout)

  const doc = new jsPDF({
    orientation: "portrait",
    unit: "mm",
    format: "a4",
  })

  const pageW = 210
  const pageH = 297

  // Center the 2x2 grid on A4
  const marginX = (pageW - BADGE_W * 2) / 2
  const marginY = (pageH - BADGE_H * 2) / 2

  // Grid positions (2 columns, 2 rows)
  const positions = [
    { x: marginX, y: marginY },
    { x: marginX + BADGE_W, y: marginY },
    { x: marginX, y: marginY + BADGE_H },
    { x: marginX + BADGE_W, y: marginY + BADGE_H },
  ]

  for (let i = 0; i < badges.length; i++) {
    const posIndex = i % 4
    if (i > 0 && posIndex === 0) {
      doc.addPage()
    }

    const pos = positions[posIndex]
    drawBadge(doc, badges[i], config, pos.x, pos.y)
  }

  return doc
}
