...
/**
 * ─── ADMIN: LEAD CAPTURE OVERVIEW ───────────────────────────────────────────
 *
 * Overview of all leads across all sponsors.
 * Filterable by event. Includes stats and CSV export.
 */
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-2xl font-bold text-[#333] mb-1">Lead Capture</h2>
          <p className="text-sm text-[#888]">
            All leads captured by sponsors across events
          </p>
        </div>
        <button
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          ...
          <StatCard
            label="Warm"
            value={stats.byInterest.warm ?? 0}
            color="text-orange-600"
            bgColor="bg-orange-50"
          />
          <StatCard
            label="Medium"
            value={stats.byInterest.medium ?? 0}
            color="text-blue-600"
            bgColor="bg-blue-50"
          />
          <StatCard
            label="Cold"
            value={stats.byInterest.cold ?? 0}
            color="text-gray-500"
            bgColor="bg-gray-50"
          />
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <div className="relative">
          ...
          <ChevronDown
            size={14}
            className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#aaa] pointer-events-none"
          />
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((lead) => {
                  const badge =
                    INTEREST_BADGE[lead.interest_level] ?? INTEREST_BADGE.medium
                  return (
                    <tr
                      key={lead.id}
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-[#777] text-xs">
                        <div className="flex items-center gap-1.5 capitalize">
                          {lead.captured_via === "qr_scan" && (
                            <ScanLine size={12} className="text-[#c9a84c]" />
                          )}
                      ...
                      <td className="px-5 py-4">
                        <select
                          ...
                          onChange={(e) =>
                            handleFollowUpChange(lead.id, e.target.value)
                          }
...
function StatCard({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-[#e0e0e0] p-4",
        bgColor ?? "bg-white"
      )}
    >
