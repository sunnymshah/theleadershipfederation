    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-white/40">Create discount codes for ticket purchases</p>
        </div>
      ...
      <div className="relative mb-6">
        ...
        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search codes…" className="w-full pl-10 pr-4 py-2.5 bg-white/[0.03] border border-white/[0.06] rounded-lg text-sm text-white placeholder-white/25 focus:outline-none focus:border-white/15 transition-colors" />
      </div>
      ...
      <div className="rounded-xl border border-white/[0.06] overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-20 text-white/30 gap-2"><Loader2 size={18} className="animate-spin" /> Loading…</div>
        ) : filtered.length === 0 ? (
          <div className="py-20 text-center">
            <Tag size={32} className="mx-auto mb-3 text-white/15" />
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.015] transition-colors">
                  ...
                  <td className="px-5 py-4 text-white/50 text-xs">{p.events?.title ?? "—"}</td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => { setEditing(p); setDrawerOpen(true); setActionError(null) }} className="p-2 rounded-md text-white/30 hover:text-white/70 hover:bg-white/[0.05] transition-colors"><Pencil size={15} /></button>
                      <button onClick={() => handleDelete(p.id)} disabled={deletingId === p.id} className="p-2 rounded-md text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors disabled:opacity-30">
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-md bg-[#0a0a0a] border-l border-white/[0.08] z-50 shadow-2xl overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-[11px] text-white/50 uppercase tracking-wider mb-1.5">Code *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[11px] text-white/50 uppercase tracking-wider mb-1.5">Value *</label>
                  <input type="number" name="discountValue" required min="1" defaultValue={editing?.discount_value ?? ""} className="w-full px-3 py-2.5 bg-white/[0.03] border border-white/[0.08] rounded-lg text-sm text-white focus:outline-none focus:border-[#c9a84c]/50 transition-colors" placeholder="20" />
                </div>
              </div>
              <div>
                <label className="block text-[11px] text-white/50 uppercase tracking-wider mb-1.5">Max Uses (blank = unlimited)</label>
                <input type="number" name="maxUses" min="1" defaultValue={editing?.max_uses ?? ""} className="w-full px-3 py-2.5 bg-white/[0.03] border border-white/[0.08] rounded-lg text-sm text-white focus:outline-none focus:border-[#c9a84c]/50 transition-colors" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div>
                ...
                <select name="active" defaultValue={editing?.active !== false ? "true" : "false"} className="w-full px-3 py-2.5 bg-white/[0.03] border border-white/[0.08] rounded-lg text-sm text-white focus:outline-none focus:border-[#c9a84c]/50 transition-colors">
