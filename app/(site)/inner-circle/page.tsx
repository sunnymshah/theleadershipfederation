...
export default async function InnerCirclePage() {
  ...
  const testimonials = items.filter(i => i.content_type === "testimonial")
              ...
              {valueProps.map((vp) => {
                ...
                return (
                  ...
                  >
                    {vp.accent && (
                      <div
                        className={`absolute top-0 right-0 w-48 h-48 rounded-full bg-gradient-to-br ${vp.accent} blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                        aria-hidden
                      />
                    )}
