  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {[
          { label: "Total", value: stats.total, color: "text-[#555]" },
          ...
          <div key={s.label} className="px-4 py-3 rounded-lg border border-[#e0e0e0] bg-white text-center">
            <div className={cn("text-xl font-bold tabular-nums", s.color)}>{s.value}</div>
            <div className="text-[10px] text-[#aaa] uppercase tracking-wider">{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between mb-5 gap-4">
        <div className="flex items-center gap-3 flex-1">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-white border border-[#e0e0e0] rounded-lg text-xs text-[#333] placeholder-[#bbb] focus:outline-none focus:border-[#ccc] transition-colors"
            />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="px-3 py-2 bg-white border border-[#e0e0e0] rounded-lg text-xs text-[#333] focus:outline-none focus:border-[#ccc] transition-colors"
          >
        ...
        <div className="flex items-center gap-2">
          {attendees.length > 0 && (
            <button
              onClick={exportCSV}
              className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[#e0e0e0] text-xs text-[#777] hover:text-[#555] hover:bg-[#fafafa] transition-colors"
            >
      ...
      <div className="rounded-xl border border-[#e0e0e0] overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-[#999] text-sm">{searchQuery || statusFilter !== "all" ? "No leads match your filters." : "No registrations yet."}</p>
            <p className="text-[#bbb] text-xs mt-1">Leads are captured automatically when visitors register on the public site.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#e0e0e0] bg-white">
                ...
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-[#888] uppercase tracking-wider">Registered</th>
                ...
                return (
                  <tr key={a.id} className="border-b border-[#eee] last:border-0 hover:bg-[#fafafa] transition-colors">
                    <td className="px-5 py-3.5">
                      ...
                      {a.designation && <div className="text-[11px] text-[#aaa]">{a.designation}</div>}
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="text-[#666] text-xs">{a.email}</div>
                      {a.phone && <div className="text-[11px] text-[#aaa]">{a.phone}</div>}
                    </td>
                    <td className="px-5 py-3.5 text-[#777] text-xs">{a.company ?? "—"}</td>
                    <td className="px-5 py-3.5 text-[#777] text-xs">{a.tickets?.name ?? "—"}</td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-[#888] text-[11px] whitespace-nowrap">{fmtDate(a.registration_date)}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        {a.status !== "checked_in" && a.status !== "cancelled" && (
                          <button
                            ...
                            className="p-1.5 rounded-md text-[#aaa] hover:text-emerald-400 hover:bg-emerald-500/10 transition-colors disabled:opacity-30"
                            title="Check In"
                          >
                            {checkingInId === a.id ? <Loader2 size={14} className="animate-spin" /> : <UserCheck size={14} />}
                          </button>
                        )}
                        <button onClick={() => { setEditing(a); setDrawerOpen(true); setActionError(null) }} className="p-1.5 rounded-md text-[#aaa] hover:text-[#555] hover:bg-[#fafafa] transition-colors"><Pencil size={14} /></button>
                        <button onClick={() => handleDelete(a.id)} disabled={deletingId === a.id} className="p-1.5 rounded-md text-[#aaa] hover:text-red-400 hover:bg-red-500/10 transition-colors disabled:opacity-30">
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-md bg-white border-l border-[#e0e0e0] z-50 shadow-2xl overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                ...
                <input type="text" name="name" required defaultValue={editing?.name ?? ""} className="w-full px-3 py-2.5 bg-white border border-[#e0e0e0] rounded-lg text-sm text-[#333] placeholder-[#bbb] focus:outline-none focus:border-[#c9a84c]/50 transition-colors" placeholder="John Doe" />
              </div>
              <div>
                <label className="block text-[11px] text-[#777] uppercase tracking-wider mb-1.5">Email *</label>
                <input type="email" name="email" required defaultValue={editing?.email ?? ""} className="w-full px-3 py-2.5 bg-white border border-[#e0e0e0] rounded-lg text-sm text-[#333] placeholder-[#bbb] focus:outline-none focus:border-[#c9a84c]/50 transition-colors" placeholder="john@company.com" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] text-[#777] uppercase tracking-wider mb-1.5">Phone</label>
                  <input type="tel" name="phone" defaultValue={editing?.phone ?? ""} className="w-full px-3 py-2.5 bg-white border border-[#e0e0e0] rounded-lg text-sm text-[#333] placeholder-[#bbb] focus:outline-none focus:border-[#c9a84c]/50 transition-colors" placeholder="+91 98765 43210" />
                </div>
                <div>
                  <label className="block text-[11px] text-[#777] uppercase tracking-wider mb-1.5">Designation</label>
                  <input type="text" name="designation" defaultValue={editing?.designation ?? ""} className="w-full px-3 py-2.5 bg-white border border-[#e0e0e0] rounded-lg text-sm text-[#333] placeholder-[#bbb] focus:outline-none focus:border-[#c9a84c]/50 transition-colors" placeholder="CTO" />
                </div>
              </div>
              <div>
              ...
              <div>
                <label className="block text-[11px] text-[#777] uppercase tracking-wider mb-1.5">Ticket Tier</label>
                <select name="ticketId" defaultValue={editing?.ticket_id ?? ""} className="w-full px-3 py-2.5 bg-white border border-[#e0e0e0] rounded-lg text-sm text-[#333] focus:outline-none focus:border-[#c9a84c]/50 transition-colors">
              ...
              {editing && (
                <div>
                  ...
                  <select name="status" defaultValue={editing.status} className="w-full px-3 py-2.5 bg-white border border-[#e0e0e0] rounded-lg text-sm text-[#333] focus:outline-none focus:border-[#c9a84c]/50 transition-colors">
                    <option value="registered">Registered</option>
                    <option value="confirmed">Confirmed</option>
                    <option value="checked_in">Checked In</option>
                    <option value="cancelled">Cancelled</option>
              ...
              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => { setDrawerOpen(false); setEditing(null) }} className="flex-1 py-2.5 rounded-lg border border-[#e0e0e0] text-sm text-[#777] hover:text-[#555] hover:bg-[#fafafa] transition-colors">Cancel</button>
